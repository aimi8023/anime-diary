...
- 解析和规范化 `q`、`year`、`season`、`tag`、`rating`、`group`、`dir`；
- 兼容旧版 `sort` 参数并映射到对应排列维度；
- 将有效条件写回 URL；
- 关键词、年份、季度、标签和最低评分的 AND 筛选；
- 排列维度：`year` 按播出年份分行、`rating` 按评分档分行（10.0、9.5…，不按年份分割）、`time` 按收录时间平铺；
- 每个维度支持 `dir=asc/desc` 升降序；
- `getYearlyRecap` 年度聚合（部数、平均分、年度之作、高频标签、季度分布）；
- 活动筛选数量（排列偏好不计入）。
