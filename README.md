...
- **高密度浏览**：按季度/评分两个维度排列，升降序可切换；每个维度一组自适应海报墙网格，全部作品直接可见、纵向滚动浏览。
...
- **季度批量入库**：按年份+季度拉取 Bangumi 放送列表，勾选看过的番一键入库（记为未评分），之后在记录工作区用“补评分”快速打星和写感想。
...
截至 2026-08-30，以下阶段均已完成并进入 `main`：
...
7. 导航搜索、紧凑海报网格和沉浸式详情；
8. 公开档案海报墙网格（取代横向滚动行）；
9. 季度批量入库、先入库后补分与"未评分"状态；
10. Bangumi 客户端可选出站代理与季度划分校准。
...
BANGUMI_PROXY=可选的代理地址，本地直连Bangumi超时时使用（如http://127.0.0.1:7897）
```
...
| `/api/bangumi/*` | 管理员 | Bangumi 搜索、季度放送列表和条目预填 |
