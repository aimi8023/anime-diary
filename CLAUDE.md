...
├── components/
│   ├── anime-card.tsx           ← 番剧卡片（统一高度，无状态标签）
│   ├── season-section.tsx       ← 季度区块（可折叠）
│   ├── star-rating.tsx          ← 星星评分（支持 0.5 步长）
│   ├── anime-form.tsx           ← 表单组件（滑块评分、年份增减）
│   ├── anime-list.tsx           ← 管理列表（支持搜索）
│   └── timer.tsx                ← 实时计时器（本站运行时间）
├── data/anime.json              ← 示例数据（本地开发用）
├── THEME-CONFIG.md              ← 透明度调整指南
...
**注意**：已移除 `status` 字段（想看/在看/看完/弃番），简化数据模型。

## 路由与权限

| 路由 | 权限 | 说明 |
|------|------|------|
| `/` | 公开 | 首页，按季度浏览，评分倒序，支持搜索和实时计时 |
| `/login` | 公开 | 密码登录页 |
| `/admin` | 需登录 | 管理页，增删改番剧，集成搜索栏 |
...
- `app/page.tsx` 使用客户端组件以支持 framer-motion 动画
...
- **卡片高度统一**：所有番剧卡片最小高度 200px，评论区域固定 40px（两行），底部信息始终对齐
- **透明度调整**：如需修改毛玻璃效果，参考 [THEME-CONFIG.md](./THEME-CONFIG.md)
